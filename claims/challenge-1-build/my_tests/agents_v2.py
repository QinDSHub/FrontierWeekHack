...
class ClaimsTriageAgentNew:
    ...
    def create(self):
        ...
        self.agent = self.client.agents.create_version(
            agent_name="claims-triage-agent-new-new",
...
class ClaimsDecisionAgentNew:
    ...
    def create(self):
        ...
        self.agent = self.client.agents.create_version(
            agent_name="claims-decision-agent-new-new",
            definition=PromptAgentDefinition(
                model=MODEL_DEPLOYMENT_NAME,
                instructions=system_prompt,
            ),
...
class DoubleCheckAgentNew:
    ...
    def create(self):
        ...
        system_prompt = """
        You are a senior claims double check reviewer for outputs from decision agent.
        Determine whether the decision agent's output indicates that the claim should be approved: Ture, False  
        Be concise and structured.
        """
        
        self.agent = self.client.agents.create_version(
            agent_name="double-check-agent-new-new",
            definition=PromptAgentDefinition(
                model=MODEL_DEPLOYMENT_NAME,
                instructions=system_prompt,
            ),
...
def main():
    ...
    triage_agent = ClaimsTriageAgentNew()
    triage_agent.create()
    ...
    print("\n=== Claims Decision Agent New ===")
    print("Creating agent...")

    decision_agent = ClaimsDecisionAgentNew()
    ...
    check_result = check_agent.run(
        ...
        "HIGH_RISK_CLAIM_BATCH:\n"
        f"{json.dumps(approve_batch, indent=2)}"
    )
    print(check_result)
