...
class ClaimsTriageAgent:
    ...
    def create(self):
        ...
        - Determine the Claim classification: normal (all metrics pass), critical (Fraud Risk Score significantly exceeds the maximum threshold), otherwise warning.
        ...
        Use ⚠️ for warning, 🔴 for critical and ✅ for normal flags.
...
class ClaimsDecisionAgent:
    ...
    def create(self):
        ...
        - Claim ID, type, and claimant name, classification
        - Determine the action: approve for normal claims, investigate for critical claims, request_documents when Completeness fails, and partial_deny otherwise.
        - Estimate urgency: low for normal, medium for warning, and high for critical claims.
...
# =============================================================================
# Claims Decision Agent (this is only for evulation in foundry portal)
# =============================================================================

class ClaimsDecisionAgentPortal:
    def __init__(self):
        self.agent = None
        self.client = None
        self.openai = None
    ...
    def create(self):
        ...
        self.agent = self.client.agents.create_version(
            agent_name="claims-decision-agent-portal",
            definition=PromptAgentDefinition(
                model=MODEL_DEPLOYMENT_NAME,
                instructions=system_prompt,
            ),
...
def main():
    ...
    print("\n=== Claims Decision Agent Portal ===")
    print("Creating agent...")

    decision_agent_portal = ClaimsDecisionAgentPortal()
    decision_agent_portal.create()
    print(f"✅ Created: {decision_agent_portal.agent.name} (version {decision_agent_portal.agent.version})")
    print("Now, you could test the evulations in Foundry portal.")
