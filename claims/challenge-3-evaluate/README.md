...
4. Choose `claims-triage-agent` or `claims-decision-agent-portal` from the dropdown
5. Select **Individual Turns** and then **Existing Dataset**
6. Click on **Upload new dataset**. 
You must enter a dataset name first — the upload stays disabled until you do. Type a name, then add the file located on `claims/challenge-3-evaluate/triage_eval_portal.jsonl` for `claims-triage-agent` or `claims/challenge-3-evaluate/decision_eval_portal.jsonl` for `claims-decision-agent-portal`, and then confirm the upload. (Because `claims-decision-agent' is for business outputs such as more details and reasoning info rather than for evulations in Foundry portal channel.)
